...
class MoorhenSynth {
    ...
    midiNoteToFrequency(midiNote) {
        return 440 * Math.pow(2, (midiNote - 69 + this.pitchBend) / 12);
    }
    ...
    playBowl(noteIndex) {
        this.playBowlAtFreq(this.getFrequency(noteIndex));
    }
    ...
    playWind(noteIndex) {
        this.playWindAtFreq(this.getFrequency(noteIndex));
    }
    ...
    playString(noteIndex) {
        this.playStringAtFreq(this.getFrequency(noteIndex));
    }
    
    playStringAtFreq(freq) {
        const now = this.audioContext.currentTime;
    ...
    playHorn(noteIndex) {
        this.playHornAtFreq(this.getFrequency(noteIndex));
    }
    
    playHornAtFreq(freq) {
        freq = freq / 2; // One octave lower
        const now = this.audioContext.currentTime;
    ...
    playMoorhen(noteIndex) {
        this.playMoorhenAtFreq(this.getFrequency(noteIndex));
    }
    
    playMoorhenAtFreq(freq) {
        freq = freq * 2; // Higher pitched bird call
        const now = this.audioContext.currentTime;
    ...
    playOystercatcher(noteIndex) {
        this.playOystercatcherAtFreq(this.getFrequency(noteIndex));
    }
    
    playOystercatcherAtFreq(freq) {
        freq = freq * 3; // Very high pitched call
        const now = this.audioContext.currentTime;
    ...
    playStick(noteIndex) {
        ...
        this.playStickAtFreq(basePitch);
    }
    
    playStickAtFreq(basePitch) {
    ...
    // Play any instrument by name (using pentatonic scale index 0-6)
    playInstrument(instrument, noteIndex) {
        const freq = this.getFrequency(noteIndex);
        this.playInstrumentAtFreq(instrument, freq);
    }
    
    // Play any instrument at a specific MIDI note (chromatic, any note)
    playInstrumentMIDI(instrument, midiNote) {
        const freq = this.midiNoteToFrequency(midiNote);
        this.playInstrumentAtFreq(instrument, freq);
    }
    
    // Play any instrument at a specific frequency
    playInstrumentAtFreq(instrument, freq) {
        switch(instrument) {
            case 'bowls':
                this.playBowlAtFreq(freq);
                break;
            case 'wind':
                this.playWindAtFreq(freq);
                break;
            case 'strings':
                this.playStringAtFreq(freq);
                break;
            case 'horn':
                this.playHornAtFreq(freq);
                break;
            case 'moorhen':
                this.playMoorhenAtFreq(freq);
                break;
            case 'oystercatcher':
                this.playOystercatcherAtFreq(freq);
                break;
            case 'stick':
                this.playStickAtFreq(freq);
                break;
        }
    }
